...
const Home = () => {
  const [messages, setMessages] = useState<Message[]>([
    {
      id: 1,
      content: `Welcome to LexAI! I'm your legal research assistant.
...
Ask me anything about law, and I'll search authoritative sources to provide you with accurate, well-cited information.`,
      isUser: false,
      type: 'message'
    }
  ]);
    ...
    if (currentMessage.trim()) {
      ...
      try {
        ...
        eventSource.onerror = (error) => {
          ...
          if (!streamedContent) {
            setMessages(prev =>
              prev.map(msg =>
                msg.id === aiResponseId
                  ? { ...msg, content: "Unable to connect to the server. Please ensure the backend is running at http://localhost:8000", isLoading: false }
                  : msg
              )
            );
          }
        ...
        setMessages(prev => [
          ...
          {
            id: newMessageId + 1,
            content: "Connection error. Please verify the server is running.",
            isUser: false,
            type: 'message',
            isLoading: false
          }
